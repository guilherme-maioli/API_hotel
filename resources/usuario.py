...
class UserRegister(Resource):
	...
	def post(self):
		...
		if not dados.get('email') or dados.get('email') is None:
			return{"message": "The field 'email' can not be left blank."}, 400
	...
	def post(cls):
		...
		if user and safe_str_cmp(user.senha, dados['senha']):
			if user.ativado:
				...
				token_de_acesso = create_access_token(user.user_id)
...
class UserConfirm(Resource):
	...
	def get(cls, user_id):
		...
		#return {"message": "User id '{}' confirmed sucessfully.".format(user_id)}
